...
public static class PropsWaitExtensions
{
    ...
    {
        return That<T, TSource>(provider, Is.EqualTo(value), timeout);
    }
    ...
    {
        return That<T[], TSource>(provider, Is.EquivalentTo(value), timeout);
    }
    ...
    {
        var constraint = new ReusableConstraint(resolveConstraint);
        return Single<T, TSource>(provider, x => Assert.That(transform(x), constraint), timeout);
    }
    ...
    {
        var constraint = new ReusableConstraint(resolveConstraint);
        return Single<T, TSource>(provider, x => Assert.That(transform(x).Get(), constraint), timeout);
    }
